...
clientId=$(terraform output -state=Terraform/terraform.tfstate -raw client_id 2>&1)
objectId=$(terraform output -state=Terraform/terraform.tfstate -raw object_id 2>&1)
clientSecret=$(terraform output -state=Terraform/terraform.tfstate -raw client_secret 2>&1)

echo "SPN Client ID: ${clientId}" | tee -a deploySynapse.log
echo "SPN Object ID: ${objectId}" | tee -a deploySynapse.log
echo "SPN Client Secret: ${clientSecret}" | tee -a deploySynapse.log
